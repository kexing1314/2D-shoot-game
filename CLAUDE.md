...
## 当前状态（2026-09-13）
- 分支 `feat/game`，HEAD `2d7a0f2`；核心游戏 + 全部试玩迭代功能已完成（状态栏/武器射程/斜射/持枪Boss+攻击节奏/受伤硬直击退/视觉升级包/低血肾上腺素/加农炮爆炸弹+冲击波圈/数值调优/换弹系统+头顶提示）；start.bat 一键开服（纯 ASCII，cmd GBK 陷阱）
- 剩余：T12 人工双窗口验收（用户以增量试玩代替）→ superpowers:finishing-a-development-branch（合并 feat/game→main 前须停下问用户）
