...
- 核心游戏 + 全部试玩迭代完成（状态栏/武器射程/斜射/持枪Boss+攻击节奏/受伤硬直击退/视觉升级包/低血肾上腺素/火箭筒命中即爆+冲击波圈/数值调优/换弹系统+头顶提示/30Hz tick/全员精灵建模/合作化（友伤关闭）+波次怪潮（难度 diff 参数预留）+15 级等级系统（levelMul 六倍率）+护盾+波间补给）
- **多地图**：shared.js `MAPS` 表 4 张（草地要塞/沙漠废墟/冰原站台/陶土峡谷，4200~5200 宽），房主大厅选图（卡片带墙布局预览）→ create 带 map key → joined 带回；地砖同包 CC0 四主题 pattern、墙材质随主题（苔石/砂岩/冰块/土坯，可破坏墙恒木箱色）、种子散布无碰撞 props；test.js 逐图断言墙界/刷新点间距
